...
settingswidget::settingswidget(QWidget *parent)
  ...
  connect(ui->diyurl, &QPushButton::clicked, this, [=, this]() {
    ...
    connect(urlwidget, &url_settingswidget::ok_text, this,
...
void settingswidget::display() {
  ...
  if (!eng_selected)
    ui->web1->setChecked(true);
  else
    ui->web2->setChecked(true);
  if (!jap_selected)
    ui->web3->setChecked(true);
  else
    ui->web4->setChecked(true);
}

void settingswidget::setpages(int i) { pages = i; }

void settingswidget::setautosavetime(int i) { auto_save_time = i; }
